...
TEST(ErrorTest, NetworkErrorMessageIsCorrect) {
    ...
    EXPECT_STREQ(networkError.what(), "Error: Network Error: Failed to connect to server");
}
...
TEST(ErrorTest, TimeoutErrorMessageIsCorrect) {
    ...
    EXPECT_STREQ(timeoutError.what(), "Error: Timeout Error: Operation timed out");
}

TEST(ErrorTest, ServiceErrorMessageIsCorrect) {
    ServiceError serviceError("Service unavailable");
    EXPECT_STREQ(serviceError.what(), "Error: Service Error: Service unavailable");
}
